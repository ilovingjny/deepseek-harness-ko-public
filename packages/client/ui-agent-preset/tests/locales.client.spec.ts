...
import { en, ko, presetDisplayText, zh } from '../src/client/locales.ts'
...
describe('preset display copy', () => {
  it.each([
  ...
  ] as const)('localizes the shipped %s preset in English, Chinese, and Korean', (id, nameKey, descriptionKey) => {
    ...
    expect(presetDisplayText(preset, translate(ko)))
      .toEqual({ name: ko[nameKey], description: ko[descriptionKey] })
  })
