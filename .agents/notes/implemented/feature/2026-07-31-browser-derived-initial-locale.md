...
- **Full BCP 47 negotiation (`Intl.LocaleMatcher`-style lookup, region and script weighting)**: the shipped locales differ by primary language, so primary-subtag matching is the whole of the correct answer; a negotiation layer would be untestable surface with no behavior to justify it.
...
- A first visit from a Chinese, English, or Korean browser lands in that language, and the Language row shows all three self-described options.
